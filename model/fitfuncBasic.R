...
make.eeg.out <- function(tr) {                               
    ...
    #alpha.iqr <- c(.1, .3, .5, .7, .9) 
    #beta.iqr <-  c(1, 3, 5, 7, 9) 
    #alpha.iqr <- c(.46, .87) ## old lax starting points (25th / 75th percentile from Daw et al. (2011)
    #beta.iqr <-  c(2.7, 7.5) ## old lax starting points
  alpha.iqr <- c(0.5)
  beta.iqr <-  c(0.5)
    ...
    ## Minimize objective function (and do this from each of 16 starting points defined by IQRs)
    fit.big <- NULL                                                             
