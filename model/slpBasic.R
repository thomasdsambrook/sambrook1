...
slpBasic <- function(st, tr, xtdo = FALSE) { 
    ...
    for(i in 1:nrow(tr)) {                  
        ...
        ## Calculate MF delta                
        s1.d.mf <- t - st$q1.mf[s1.act]     ####Q to self, I can see that teh value of t on this row is defined earlier, as is s1.act but how is q1.mf so contsrained?
        ## Calculate stuff the model doesn't use (but might be handy as neural regressors)...currently empty
        if(xtdo) {     
            }
        ## Update S1 QMF
